...
template<class T>
class COleDropSourceWnd : public COleDropSource
{
	T* m_pTarget;

public:
	COleDropSourceWnd(T* pTarget)
		:m_pTarget(pTarget)
	{
	}
	...
	{
		if (m_pTarget != NULL)
			return m_pTarget->OnDragGiveFeedback(dropEffect);
		else
			return COleDropSource::GiveFeedback(dropEffect);
	}

private:
	COleDropSourceWnd(const COleDropSourceWnd&);
	COleDropSourceWnd& operator=(const COleDropSourceWnd&);
};
//------------------------------------------------------------------------
//! @endcond INTERNAL
//------------------------------------------------------------------------
...
BOOL CGridListCtrlEx::OnBeginDrag(NMHDR* pNMHDR, LRESULT* pResult)
{
	...
	COleDropSourceWnd<CGridListCtrlEx> oleDropSource(this);
	DROPEFFECT dropEffect = DoDragDrop(oleDataSource, &oleDropSource);
...
//! @param lpRectStartDrag Pointer to the rectangle that defines where the drag actually starts
//! @param pDropSource Points to a drop source.
//! @return Drop effect generated by the drag-and-drop operation; otherwise DROPEFFECT_NONE
//------------------------------------------------------------------------
DROPEFFECT CGridListCtrlEx::DoDragDrop(COleDataSource& oleDataSource, COleDropSource* pDropSource, DWORD dropEffects, LPCRECT lpRectStartDrag)
{
	...
	DROPEFFECT dropEffect = oleDataSource.DoDragDrop(dropEffects, lpRectStartDrag, pDropSource);
	GlobalFree(hglbCopy);
	return dropEffect;
}
...
void CGridListCtrlEx::deleteItemEx(int nItem)
{
...
void CGridListCtrlEx::ReleaseMemory()
{
	MEMORY_MANAGER_VECTOR::iterator iter;

	iter = m_memoryManager.begin();
	for (iter; iter != m_memoryManager.end(); iter++)
	{
		delete[](*iter)->arrpsz;
		delete[](*iter)->crBak;
		delete[](*iter)->crText;
...
void CGridListCtrlEx::OnDrawItem(int nIDCtl, LPDRAWITEMSTRUCT lpDrawItemStruct)
{
